# Define the set of learning rates and normalization types
norm_type=$1
learning_rates=2e-3
export NORM_TYPE=$norm_type

# Function to run a single training task

echo "Training with learning rate: $learning_rates, norm type: $norm_type on GPU $gpu"

CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node 4 --master_port=29500 torchrun_main.py \
    --model_config configs/llama_71m_24L.json \
    --seed 42 \
    --lr $learning_rates \
    --max_length 1024 \
    --batch_size 64 \
    --total_batch_size 512 \
    --num_training_steps 100000 \
    --warmup_steps 2000 \
    --weight_decay 0.1 \
    --dtype bfloat16 \
    --eval_every 1000 \
    --optimizer adamw \
    --scheduler wsd \
    --grad_clipping 1.0 \
    --prores_warmup_steps 24000 \
    --run_name "71m_24L_res_${norm_type}_lr${learning_rates}" \
    --save_dir "71m_24L_res_${norm_type}_lr${learning_rates}"