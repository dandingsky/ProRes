...
def get_scheculer(
    ...
    *,
    ...
    if scheduler_type == "cosine_single":
        return get_single_cosine_schedule_with_min_lr(
            optimizer,
    ...
    if scheduler_type == "wsd":
        # warmup stable decay
        return get_warmup_stable_decay_scheduler(
            optimizer,
            num_training_steps=num_training_steps,
...
def get_warmup_stable_decay_scheduler(
    optimizer, 
    ...
    """
    Scheduler: linear warmup -> stable max LR -> linear decay to zero.

    Args:
        optimizer: torch.optim.Optimizer
        warmup_steps: steps to linearly increase LR from 0 to max_lr
        stable_steps: steps to maintain max_lr
        decay_steps: steps to linearly decay LR from max_lr to 0
    ...
    def lr_lambda(step):
        if step < warmup_steps:
            ...
            return remaining_steps / decay_steps
        else:
            return 0.0
...
def get_single_cosine_schedule_with_min_lr(optimizer, num_zero_steps, num_warmup_steps, num_training_steps, cycle_length, min_lr_ratio=0.1, last_epoch=-1):
    assert cycle_length is not None or num_training_steps is not None, "You must specify either cycle_length or num_training_steps"
    ...
    lr_lambda = partial(
        _get_single_cosine_schedule_with_min_lr_lambda,
        num_zero_steps=num_zero_steps,
        num_warmup_steps=num_warmup_steps,
        cycle_length=cycle_length,
        min_lr_ratio=min_lr_ratio,
...
def _get_single_cosine_schedule_with_min_lr_lambda(current_step, *, num_zero_steps, num_warmup_steps, cycle_length, min_lr_ratio):
    assert 0 <= min_lr_ratio <= 1.0, "min_lr_ratio must be in [0,1]"

    # do not cycle
    # stay min lr once reached
    if current_step >= cycle_length:
        return min_lr_ratio
    
    cycle_step = current_step % cycle_length
    # remain zero initialy
    if cycle_step < num_zero_steps:
        return 0
    elif cycle_step < num_warmup_steps + num_zero_steps:
        if current_step != cycle_step:
            if cycle_step < 2:
                return 1e-7
        return float(cycle_step - num_zero_steps) / float(max(1, num_warmup_steps))

    progress = float(cycle_step - num_warmup_steps - num_zero_steps) / float(max(1, cycle_length - num_warmup_steps - num_zero_steps))
    cosine_decay = 0.5 * (1.0 + math.cos(math.pi * progress))
